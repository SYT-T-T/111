            ...
            for epoch in trange(1, self.epochs + 1, desc=f"IGAN Training Epochs"):
                ...
                with torch.no_grad():
                    ...
                    for _batch in train_loader:
                        X_for_D, _, _ = _batch
                        break
                    if X_for_D is not None:
                        X_for_D = X_for_D.to(self.device)
                        fake = self.generator(X_for_D)
                        D_fake = self.discriminator(fake)
                        print(f'[IGAN] Epoch {epoch}: D_real mean={D_real.mean().item():.4f}, std={D_real.std().item():.4f} | D_fake mean={D_fake.mean().item():.4f}, std={D_fake.std().item():.4f}')
                        # === 判别器输出监控, 推理期间 ===
